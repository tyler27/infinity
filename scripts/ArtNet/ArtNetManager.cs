...
namespace LazerSystem.ArtNet
{
    ...
    public partial class ArtNetManager : Node
    {
        ...
        /// <summary>Syncs projector IP addresses from ProjectorConfig resources.</summary>
        public void SyncProjectorAddresses(Godot.Collections.Array<LazerSystem.Core.ProjectorConfig> projectors)
        {
            _projectorAddresses = new string[projectors.Count];
            for (int i = 0; i < projectors.Count; i++)
            {
                _projectorAddresses[i] = projectors[i]?.IpAddress ?? "";
            }
        }

        /// <summary>Clears the discovered nodes list for re-scanning.</summary>
        public void ClearDiscoveredNodes()
        {
            _discoveredNodes.Clear();
        }

        /// <summary>Sets the broadcast address at runtime.</summary>
